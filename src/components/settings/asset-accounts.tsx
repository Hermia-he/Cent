...
import { numberToAmount, amountToNumber } from "@/ledger/bill";
...
function Form({ onCancel }: { onCancel?: () => void }) {
    ...
    const [editingAccount, setEditingAccount] = useState<Partial<AssetAccount> | null>(null);
    ...
    const toSave = async () => {
        ...
        await useLedgerStore.getState().updateGlobalMeta((prev) => {
            ...
            if (editingAccount.id) {
                // Edit existing
                prev.assets = prev.assets.map((a) => (a.id === editingAccount.id ? newAccount : a));
            } else {
                // Add new
            ...
            <div className="flex flex-col h-full overflow-hidden">
                ...
                <div className="px-6 py-4 flex-shrink-0">
                    <div className="bg-gradient-to-br from-rose-50 to-orange-50 dark:from-stone-900 dark:to-orange-950/20 border border-rose-100/50 dark:border-rose-950/30 rounded-2xl p-5 shadow-sm">
                        <div className="text-xs text-stone-500 dark:text-stone-400 font-medium">期初总存款 (共同资产)</div>
                    ...
                    {editingAccount ? (
                        ...
                        <div className="flex flex-col gap-4 border border-rose-100 dark:border-stone-800 p-5 rounded-2xl bg-stone-50/50 dark:bg-stone-950/30">
                            <h3 className="font-semibold text-sm text-stone-700 dark:text-stone-200">
                                {editingAccount.id ? t("edit-account") : t("add-account")}
                            </h3>

                            {/* 账户名称 */}
                            <div className="flex flex-col gap-1.5">
                                <label className="text-xs text-stone-500 font-bold">{t("account-name")}</label>
                                <input
                                    type="text"
                                ...
                                />
                                {nameError && <span className="text-[10px] text-rose-500 font-bold">{nameError}</span>}
                            </div>

                            {/* 账户类型 */}
                            <div className="flex flex-col gap-1.5">
                                <label className="text-xs text-stone-500 font-bold">{t("account-type")}</label>
                                <select
                            ...
                            {/* 期初余额 */}
                            <div className="flex flex-col gap-1.5">
                                <label className="text-xs text-stone-500 font-bold">{t("initial-amount")} (¥)</label>
                                <input
                                    type="number"
                                    value={editingAccount.initialAmount !== undefined ? editingAccount.initialAmount : ""}
                                    onChange={(e) => setEditingAccount({ ...editingAccount, initialAmount: e.target.value as any })}
                                    className="flex h-10 w-full rounded-xl border border-stone-200 dark:border-stone-800 bg-white dark:bg-stone-900 px-3 py-1 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-400"
                                    placeholder="0.00"
                                    step="0.01"
                                    min="0"
                                />
                                {amountError && <span className="text-[10px] text-rose-500 font-bold">{amountError}</span>}
                            </div>
                            ...
                            <div className="flex gap-2 justify-end mt-2">
                                ...
                                <Button size="sm" className="bg-rose-500 hover:bg-rose-600 text-white" onClick={toSave}>
                                    {t("save")}
                                </Button>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col gap-3">
                            <div className="flex justify-between items-center pb-1">
                                <span className="text-xs text-stone-500 font-bold">已存账户 ({assets.length})</span>
                                <Button
                                    size="sm"
                                    className="bg-rose-100 hover:bg-rose-200 text-rose-600 font-bold"
                                    onClick={() => setEditingAccount({ name: "", type: "card", initialAmount: 0 })}
                                >
                                    + {t("add-account")}
                                </Button>
                            </div>
                            ...
                            {assets.length === 0 ? (
                                ...
                                <div className="flex flex-col gap-2.5">
                                    {assets.map((account) => {
                                        const typeObj = accountTypes.find((t) => t.value === account.type);
                                        const typeLabel = typeObj ? typeObj.label : t("other");
                                        const typeIcon = 
                                            account.type === "card" ? "💳" :
                                            ...
                                            >
                                                ...
                                                <div className="flex items-center gap-4">
                                                    <span className="text-sm font-extrabold text-stone-850 dark:text-stone-100">
                                                        ¥{amountToNumber(account.initialAmount).toFixed(2)}
                                                    </span>
                                                    <div className="flex gap-1.5">
                                                        <button
                                                            ...
                                                            onClick={() => setEditingAccount({
                                                                ...
                                                                name: account.name,
                                                                type: account.type,
                                                                initialAmount: amountToNumber(account.initialAmount),
                                                            })}
...
export const [AssetAccountsProvider, showAssetAccounts] = createConfirmProvider(Form, {
